...
|デバイスピッカー |HoloDevicesFlow \ _cw5n1h2txyewy \!HoloDevicesFlow |
...
> [!NOTE]  
> 次の例では、マルチアプリのキオスクについて説明します。 1つのアプリのキオスクは同様の方法で動作しますが、キオスクのエクスペリエンスを取得できるのは1つのユーザーアカウントだけです。

**例 1**

デバイスとユーザーの両方に1つのグループ (グループ 1) を使用します。 1つのデバイスとユーザー A、B、C はこのグループのメンバーです。 キオスク構成プロファイルは、次の手順で構成します。  

- **ユーザーログオンの種類**: グループ1
- **割り当て済みグループ**: グループ1

最初にどのユーザーがデバイスにサインインしているかにかかわらず (そのため、既定のエクスペリエンスまたは OOBE を通過する)、キオスクの構成がデバイスに展開されます。 ユーザー A、B、および C では、すべてのデバイスにサインインして、キオスクのエクスペリエンスを利用することができます。

**例 2**

さまざまなキオスクエクスペリエンスが必要な2つの異なるベンダーにデバイスをコントラクトアウトします。 どちらのベンダーもユーザーを所有しており、すべてのユーザーが自分のベンダーと他のベンダーの両方からキオスクにアクセスできるようにする必要があります。 グループを構成するには、次の手順を実行します。
...
次の設定の2つのキオスク構成プロファイルを作成します。

- キオスクプロファイル 1:
   - **ユーザーのログオンの種類**: ユーザーグループ
   - **割り当て済みグループ**: デバイスグループ1

- キオスクプロファイル 2:
   - **ユーザーのログオンの種類**: ユーザーグループ
   - **割り当て済みグループ**: デバイスグループ2

これらの構成では、次のような結果が得られます。

- ユーザーがデバイス1またはデバイス2にサインインすると、Intune によってそのデバイスにキオスクプロファイル1が展開されます。
- ユーザーがデバイス3またはデバイス4にサインインすると、Intune によってそのデバイスにキオスクプロファイル2が展開されます。
- ユーザー A とユーザー B は、4つのデバイスのいずれかにサインインできます。 デバイス1またはデバイス2にサインインすると、ベンダー1のキオスクエクスペリエンスが表示されます。 デバイス3またはデバイス4にサインインした場合は、ベンダー2のキオスクエクスペリエンスが表示されます。

#### プロファイルの競合

2つ以上のキオスク構成プロファイルが同じデバイスをターゲットとしている場合、それらは競合します。 Intune で管理されているデバイスの場合、Intune は競合するプロファイルを適用しません。
...
|自動的に展開する      | なし            | なし                   | あり  |
|展開の速度            | すばやく       | Fast (高速)                 | スロー (低速) |
|Scale で展開する | 推奨されない    | 推奨されない        | 推奨 |
