...
> [!IMPORTANT]
> キオスクモードは、デバイスに Windows ホログラフィック for Business が含まれている場合にのみ使用できます。 すべての HoloLens 2 デバイスは、一般法人向け Windows ホログラフィックに付属しており、その他のエディションはありません。 すべての HoloLens 2 デバイスは、ボックスからキオスクモードを実行できます。
>
> HoloLens (第1世代) デバイスは、OS ビルドと OS エディションの両方でアップグレードする必要があります。 ここでは、 [Windows ホログラフィック For Business](hololens1-upgrade-enterprise.md) edition への HoloLens (第1世代) の更新について詳しく説明します。 HoloLens (第1世代) デバイスでキオスクモードを使うように更新するには、デバイスが Windows 10、バージョン1803、またはそれ以降のバージョンを実行していることを確認する必要があります。 HoloLens (第1世代) デバイスを既定のビルドに復元するために Windows デバイス回復ツールを使っている場合、または最新の更新プログラムがインストールされている場合は、デバイスを構成することができます。
