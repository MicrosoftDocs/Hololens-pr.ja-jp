...
この最新リリースは、バージョン21h1 の月単位の更新プログラムですが、今回は新機能を含めています。そのため、メジャービルド番号は変わらず、Windows Update はバージョン 21h1 (ビルド 20348) の月単位のリリースを示します。 このリリースを21h1 として参照しているにもかかわらず、HoloLens 2 設定には21h1 が表示されます。 21H2 を受信したことを確認するには、バージョン番号が20348.1432 以上であることを確認してください。 設定 > [バージョン情報] 画面でビルド番号を確認すると、利用可能な最新のビルド 20348.1432 + が表示されていることを確認できます。
...
| 機能                 | 説明                | ユーザーまたはシナリオ |
|-------------------------|----------------------------|--------------|
| [移動プラットフォーム モード](#moving-platform-mode) | プラットフォームモードベータの移行について説明します。これにより、構成されている場合、船舶を使用して、動的な動きが発生している大海の大規模な海での HoloLens 2 | All |
...
| [修正プログラムと機能強化](#fixes-and-improvements) | HoloLens の修正と改善。 | All |
...
[Web アプリのインストール](#install-web-apps) | 新しいブラウザーを使用HoloLens 2、Microsoft Office Web アプリを Microsoft Edgeします。 | エンド ユーザー |
...
[フォールバック診断の構成](#configuring-fallback-diagnostics-via-settings-app) | 設定 App でのフォールバック診断動作の設定。 | IT 管理者 |
[近くのデバイスと共有する](#share-things-with-nearby-devices) | ファイルまたは URL を PC にHoloLensファイルまたは URL を共有します。 | All |
...
このリリースでは、新しいバージョンの新しい 設定します。 新しい設定アプリには、サウンド、電源とスリープ、ネットワークとインターネット、アプリ、アカウント、コンピューターの簡単操作などの領域における HoloLens 2 の新機能と拡張設定が含まれています。
...
- [設定ページで、カラー プロファイルが最後に変更された日時を示すステータス文字列は、そのページのページを再度読み込むまで設定。
...
以前のビルドでは、デバイスにキオスク構成 (グローバル割り当てアクセスと AAD グループ メンバー割り当てアクセスの両方の組み合わせ) がある場合、AAD グループ メンバーシップが[](hololens-kiosk.md#issue---no-apps-are-shown-in-start-menu-in-kiosk-mode)失敗した場合、ユーザーには "何も表示されません" メニューが表示されます。
...
| [ユーザー設定ページの表示を有効HoloLens 2](hololens-release-notes.md#enabled-settings-page-visibility-for-hololens-2)      | アプリに表示されるページを選択設定ポリシー。             |
...
目の視線入力データまたは非常に正確なホログラムの配置を必要とするエクスペリエンスの場合は、未調整のユーザーに視線追跡の調整を実行することをお勧めします。 これは、目の追跡の調整プロンプトからアクセスするか、スタート メニューから 設定 アプリを起動し、[ システム >**調整 >** 目の調整 > [ 目の調整の実行] を選択することでアクセスできます。
...
oobe の実行中に HoloLens 2 を wi-fi に接続すると、oobe によってデバイスの自動操縦プロファイルが確認されます。 見つかった場合は、残りの AAD 参加および登録フローを完了するために使用されます。 つまり、イーサネットを USB-C または Wi-Fi USB-C アダプターに使用することは、もはや必須ではありませんが、OOBE の開始時に提供される場合は引き続き機能します。 [HoloLens 2 デバイスの自動操縦の](hololens2-autopilot.md)詳細については、こちらをご覧ください。
...
Windows Holographic バージョン20h2 の HoloLens 2 デバイスに対して、新しい mixed reality ポリシーが作成されました。 新しい制御可能な設定には、明るさの設定、音量の設定、混合 reality キャプチャでのオーディオ記録の無効化、診断を収集するタイミングの設定、グループメンバーシップキャッシュの AAD が含まれます。  
...
- オフラインキオスクが有効になり、AAD グループで最大60日間使用できるようになりました。
...
1. 手順 4 と 5 は、他の Azure AD ユーザー N に対して繰り返し実行できます。ここで重要な点は、すべての Azure AD ユーザーがインターネットを使用してデバイスにサインインする必要がある点です。少なくとも 1 回は、キオスク構成が対象の Azure AD グループのメンバーと判断できます。
...
新しく有効にされたポリシー。このポリシーを使用すると、デバイスの管理オプションHoloLens 2できます。
...
これらの更新プログラム ポリシーの詳細と、デバイスでそれらを使用するHoloLensについては、「更新プログラムの管理」[をHoloLensしてください](hololens-updates.md)。

### <a name="enabled-settings-page-visibility-for-hololens-2"></a>ユーザー設定ページの表示を有効HoloLens 2

- 設定 アプリでの UI コントロールの増加。ページの選択が限られている場合は混乱する可能性があります。
...
アプリでカスタマイズできるページ設定については、HoloLens 2 URI に関するページ[設定参照してください](settings-uri-list.md)。
...
研究者は現在、HoloLens デバイス上で調査モードを有効にし、これらの外部に向いている未加工のイメージ センサー ストリームにアクセスできます。 また、HoloLens 2の研究モードでは、加速度計、ジャイロスコープ、および磁力計の測定値にもアクセスできます。 ユーザーのプライバシーを保護するために、生の視線追跡カメラ画像は調査モードでは使用できませんが、視線入力の方向は既存の API を通じて使用できます。
...
### <a name="improvements-and-fixes-in-the-windows-holographic-version-20h2-update"></a>Holographic バージョン 20H2 Windowsの機能強化と修正:
...
- このリリースには、ネットワーク プロキシを使用したインターネット検出の失敗に対処するための NCSI プロキシ検出の修正が含まれています。 NCSI は、コンピュータープロキシとプロファイルごとのプロキシを使用して、インターネット接続を検出できます。 ユーザーごとのプロキシは、将来のリリースでは NCSI によってサポートされる予定です。
...
|       ダークアプリモード                              |          ダーク モードとライト モードの両方をサポートするアプリで使用できるダーク アプリ モード。表示エクスペリエンスが向上します        |
...
詳細については[、Autopilot Windows評価ガイドに関するHoloLens 2覧ください](hololens2-autopilot.md)。
...
一部のユーザーは、HoloLensまたは学校環境内の他のユーザーとデバイスを共有します。 そのため、ユーザーは長いユーザー名とパスワードを入力せずに簡単に行える必要があります。 Fast Identity Online (FIDO) を使用すると、組織内のすべてのユーザー (Azure AD テナント) は、ユーザー名やパスワードを入力せずに HoloLens にシームレスにサインインできます。
...
プロビジョニング パッケージを使用すると、HoloLensではなく、構成ファイルを使用して構成を設定HoloLens out-of-box experience。 以前は、プロビジョニング パッケージを内部メモリの HoloLensする必要があります。 USB ドライブに接続して、複数の HoloLens デバイスで再利用しやすくし、デバイスを並列でプロビジョニングできます。 プロビジョニング パッケージでデバイス管理に登録するフィールドもサポートされるので、プロビジョニング後に手動でセットアップする必要はありません。
...
ビジネスアプリの MDM アプリの展開と管理は、アプリケーションの開発に不可欠HoloLens。 管理者とユーザーは、監査と診断のためにアプリのインストール状態を表示する必要があります。 このリリースでは、アカウントアクセスの詳細を「設定または学校」に追加しました。アカウント  >    >    >  **情報 をクリック**  >  **します**。
...
USB 経由で HoloLens 2 にテザリングされている場合に、5G/LTE フォンや Wi-Fi ホットスポットなどの特定のモバイル ブロードバンド デバイスを有効にするためにサポートが追加されました。 これらのデバイスは、別のイーサネット **接続としてネットワーク** 設定に表示されます。 (外部ドライバーを必要とするモバイル ブロードバンド デバイスはサポートされていません)。この機能により、テザリングWi-Fi十分なパフォーマンスWi-Fi高帯域幅接続が可能になります。 サポートされている USB デバイスの詳細については、「Connect[と USB-C Bluetoothに関するページを参照してください](hololens-connect-devices.md)。  
