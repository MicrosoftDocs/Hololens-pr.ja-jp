---
title: HoloLens のユーザー id とログインを管理する
...
ms.locfileid: "126033252"
---
# <a name="manage-user-identity-and-login-for-hololens"></a>HoloLens のユーザー id とログインを管理する
...
## <a name="lets-talk-about-setting-up-user-identity-for-hololens-2"></a>HoloLens 2 のユーザー id を設定する方法について説明します。

他の Windows デバイスと同様に、HoloLens は常にユーザーコンテキストで動作します。 常にユーザー id が存在します。 HoloLens は、Windows 10 デバイスとほぼ同じ方法で id を扱います。 セットアップ中にサインインすると、アプリとデータを格納する HoloLens のユーザープロファイルが作成されます。 また、このアカウントは、Windows account Manager api を使用して、Edge や Dynamics 365 Remote Assist などのアプリにシングルサインオンを提供します。 

HoloLens では、いくつかの種類のユーザー id がサポートされています。 これら3種類のアカウントのいずれかを選択できますが、デバイスの管理には最適であるため、Azure AD を強くお勧めします。 複数のユーザーをサポートしているのは、Azure AD アカウントだけです。 

| ID の種類 | デバイスごとのアカウント数 | 認証オプション |
| --- | --- | --- |
| [Azure Active Directory](/azure/active-directory/)<sup>1</sup>  | 64 | <ul><li>Azure web 資格情報プロバイダー</li><li>Azure Authenticator アプリ</li><li>生体認証 (虹彩) &ndash; HoloLens 2<sup>2</sup>のみ </li><li>FIDO2 セキュリティキー</li><li>&ndash;HoloLens (第1世代) に対しては省略可能です。 HoloLens 2 には必須です</li><li>Password</li></ul> |
| [Microsoft アカウント (MSA)](/windows/security/identity-protection/access-control/microsoft-accounts) | 1 | <ul><li>生体認証 (虹彩) &ndash; HoloLens 2 のみ</li><li>&ndash;HoloLens (第1世代) に対しては省略可能です。 HoloLens 2 には必須です</li><li>Password</li></ul> |
| [ローカルアカウント](/windows/security/identity-protection/access-control/local-accounts)<sup>3</sup> | 1 | Password |
...
> [!IMPORTANT]
> 3-ローカルアカウントは、 [OOBE 中にプロビジョニングパッケージを使用](hololens-provisioning.md#apply-a-provisioning-package-to-hololens-during-setup)してデバイスにのみ設定できます。このアカウントは、後で設定アプリに追加することはできません。 既に設定されているデバイスでローカルアカウントを使用する場合[は、デバイスを更新またはリセット](hololens-recovery.md)する必要があります。

## <a name="setting-up-users"></a>ユーザーの設定

HoloLens に新しいユーザーを設定するには、2つの方法があります。 最も一般的な方法は、HoloLens の非インボックスエクスペリエンス (OOBE) 中です。 Azure Active Directory を使用する場合、他のユーザーは、Azure AD の資格情報を使用して OOBE 後[にログインでき](#setting-up-multi-user-support-azure-ad-only)ます。 OOBE 中に MSA またはローカルアカウントを使用して最初にセットアップされた HoloLens デバイスでは、複数のユーザーがサポートされません。 「 [HoloLens (第1世代)](hololens1-start.md) 」または「 [HoloLens 2](hololens2-start.md)のセットアップ」を参照してください。
...
Windows のデスクトップバージョンと同様に、他の web アカウントの資格情報を HoloLens アカウントにリンクすることができます。 このようなリンクを使用すると、アプリ間またはアプリ内でのリソースへのアクセスが容易になり (ストアなど)、個人用リソースと職場リソースへのアクセスを組み合わせることができます。 アカウントをデバイスに接続した後、デバイスをアプリに使用するアクセス許可を付与して、各アプリに個別にサインインする必要がないようにすることができます。
...
### <a name="multiple-users-are-listed-on-sign-in-screen"></a>サインイン画面に複数のユーザーが表示される

以前にサインイン画面には、最近サインインしたユーザーと "その他のユーザー" エントリポイントのみが表示されていました。 複数のユーザーがデバイスにサインインしても十分ではないというお客様からのフィードバックを受信しました。 ユーザー名を再入力する必要がありました。
...
[ **その他のユーザー** ] メニューの [ **その他の** ユーザー] ボタンをクリックすると、デバイスに最後にサインインしたユーザーが表示されます。 このボタンを選択すると、このユーザーのサインイン画面に戻ります。
...
アプリが NTLM、Basic、または Kerberos などの他の種類の認証を使用している場合は、 [Windows 資格情報 UI](/uwp/api/Windows.Security.Credentials.UI)を使用して、ユーザーの資格情報を収集、処理、および保存できます。 これらの資格情報を収集するためのユーザーエクスペリエンスは、他のクラウド駆動型のアカウント割り込みと似ており、2D アプリの上部に子アプリとして表示されるか、または UI を表示するために Unity アプリを一時的に中断します。
...
Windows Hello for Business (PIN を使用したサインインをサポート) は HoloLens (第1世代) でサポートされています。 HoloLens で Windows Hello for Business PIN サインインを許可するには、次のようにします。
...
![虹彩を設定します。](./images/setup-iris.png)
...
### <a name="how-does-the-type-of-account-affect-sign-in-behavior"></a>アカウントの種類はサインイン動作にどのように影響しますか。

サインインにポリシーを適用する場合、ポリシーは常に尊重されます。 ログインのポリシーが適用されていない場合は、次の各勘定科目の種類の既定の動作が使用されます。
