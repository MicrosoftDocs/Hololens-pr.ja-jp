...
| 証明書をインストールおよび削除する                      | ユーザーは、証明書ビューアーを使って証明書をインストールおよび削除することができます。                       | 19041.1361 +                 |
| USB からの自動起動プロビジョニング                    | OOBE は USB ドライブのプロビジョニングパッケージを正常に検出します。                                 | 19041.1361 +                 |
| OOBE でプロビジョニングパッケージを自動確認する           | OOBE でプロビジョニングパッケージを自動的に適用します。                                            | 19041.1361 +                 |
| Wi-fi 接続と自動操縦の使用                 | イーサネットアダプターを必要とせずに、デバイス Wi-fi から自動操縦を使用します。                            | 19041.1364 +                 |
...
| キオスクのゲスト自動ログオン                        | キオスクモードでの自動ログオンを使用できるようにします。                        | 19041.1361 +                 |
| 障害処理のためのキオスクモードの動作の変更 | キオスクモードのエラーが処理されるようになった変更。                                             | 19041.1356 +                 |
| 更新プログラムの機能強化と修正                 | 更新プログラムのその他の解決策。                                                               | 19041.1361 +                 |
...
Windows Insider ビルド 19041.1346 + では、HoloLens 2 設定アプリで証明書ビューアーを追加しています。 証明書のインストールでは、現在、.cer および .crt ファイルがサポートされています。 デバイスの所有者は、ローカルコンピューターおよび現在のユーザーに証明書をインストールできます。 他のすべてのユーザーは、現在のユーザーにのみインストールできます。 ユーザーは、[設定] UI から直接インストールされた証明書のみを削除することができます。 証明書が他の方法でインストールされている場合は、同じメカニズムでも削除する必要があります。
...
### 証明書をインストールおよび削除する
Windows Insider リリース19041.1361 以降では、設定アプリを使用して、HoloLens 2 で直接証明書をインストールしたり、削除したりすることができます。 証明書のインストールでは、現在、.cer および .crt ファイルがサポートされています。 デバイスの所有者は、ローカルコンピューターおよび現在のユーザーに証明書をインストールできます。 他のすべてのユーザーは、現在のユーザーにのみインストールできます。 ユーザーは、[設定] UI から直接インストールされた証明書のみを削除することができます。 証明書が他の方法でインストールされている場合は、同じメカニズムでも削除する必要があります。

#### 証明書ビューアーを使って証明書をインストールするには、次の操作を行います。 
1. [設定]、[**アプリ**の  ->  **更新とセキュリティ**  ->  **証明書**] の順に移動し、[**証明書のインストール**] を選択します。 
1. ファイルピッカーの操作環境から .cer ファイルを選びます。
1. [ローカルコンピューター] (または証明書を持っているユーザー) を選びます。
1. **ルート**を証明書ストア (または証明書を設定するストア) として選びます。 
1. **[インストール]** をクリックします。

これで、証明書がデバイスにインストールされます。

#### 証明書ビューアーを使って証明書を削除するには、次の操作を行います。 
1. [**設定**]  ->  **、[アプリの更新とセキュリティ**  ->  **証明書**] の順に移動します。
1. 検索ボックスに名前で証明書を検索します。
1. 証明書を選びます。
1. [**削除**] をクリック
1. メッセージが表示されたら、[はい] を選び、確認を求められた場合に選択します。

![証明書の UI を使用して証明書をインストールする方法を示す図](images/hololens-install-certificate.jpg)

#### 既知の問題 
インストールフロー中の、ファイルピッカーから証明書を選択した後、インストールダイアログの UI に選択された証明書ファイルが表示されない問題を調査しています。 ファイルを選択したら、ダイアログにファイルが表示されていない場合でも、インストールを続けることができます。 
