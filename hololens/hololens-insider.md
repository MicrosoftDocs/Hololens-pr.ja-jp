...
サインインしている AAD ユーザーの AAD グループ メンバーシップが正常に決定されない場合、グローバル キオスク構成はスタート メニュー (存在する場合) に使用され、それ以外の場合は、ユーザーに空のスタート メニューが表示されます。 空のスタート メニューは直接設定できる構成ではありませんが、この新しい処理は、構成に適用される場合や、割り当てられたアクセス構成に対して新しい調整を行う場合など、キオスクを使用している場合のサポート部門に通知する場合があります。
